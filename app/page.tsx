      ...
      <div className='grid grid-cols-1 w-full md:grid-cols-2 lg:grid-cols-3 gap-8 grid-flow-dense'>
        {certs.map((cert) => (
          <Link href={`/Certifications/${cert.id}`} key={cert.id}>
            <Card className='bg-violet-100 text-violet-800 dark:bg-slate-900 dark:text-violet-200 shadow-lg shadow-violet-300 dark:shadow-slate-400 border-none flex flex-col justify-center hover:scale-110 transition-all duration-700'>
              <CardHeader>
